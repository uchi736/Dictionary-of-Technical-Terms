...
**役割:**
- 用語とその定義を分析し、それが専門用語か一般用語かを判定する
- 単に用語名だけでなく、定義の内容を重視して判断する

**判定基準:**

**専門用語と判定する場合:**
1. **技術用語**: 特定の技術分野に固有の用語（例: アンモニア燃料エンジン、GHG）
2. **業界用語**: 特定の業界で使われる専門的な用語（例: 舶用エンジン、燃料噴射弁）
3. **独自定義**: 一般的な用語でも、その文脈で独自の専門的定義を持ち、かつ複合語または修飾語付き（例: 「達成」が「排出削減目標の達成」という特定の意味を持つ場合）
4. **固有名詞**: 製品名、システム名、規格名など（例: L28ADF）

**一般用語と判定する場合:**
1. **辞書的意味のみ**: 一般的な辞書に載っている意味で使われている（例: 「以上」「最大」）
2. **汎用ビジネス用語**: どの業界でも使われる一般的なビジネス用語で、独自定義がない（例: 「目標」「必要」が単に一般的な意味で使われている場合）
3. **接続詞・助詞的表現**: 文法的機能を果たす表現
4. **単体の一般名詞**: 「ガス」「燃料」「エンジン」「船」など、単体で抽出された一般的な名詞で、文脈説明のみの定義（例: 「ガス: アンモニアガスや燃料ガスとして使用される」は一般用語）

**重要な注意点:**
- 用語名が一般的でも、**定義が専門的・独自的で、かつ複合語や修飾語付きなら専門用語**
- 用語名が専門的でも、**定義が一般的・曖昧なら要検討**
- **単体の基本名詞（ガス、燃料、船など）は原則として一般用語**。ただし、定義に独自の技術的意味（化学式、規格、特殊な製法など）が含まれる場合は例外
...
{{
  ...
  "confidence": 0.0-1.0,
  "reason": "判定理由（定義のどの部分が専門的/一般的か具体的に）"
}}
```
"""
